...
const BabyAnalysisChart = ({ recordings }) => {
  ...
  const generateDateRange = () => {
    ...
    switch (timeRange) {
      ...
      case "week":
        ...
        ).reverse(); // 'EEEE' gives full day names
      case "month":
      ...
      case "year":
        ...
        return Array.from({ length: 12 }, (_, i) =>
          format(subMonths(today, i), "MMMM")
        ).reverse(); // 'MMMM' gives month names
      default:
        return Array.from({ length: 7 }, (_, i) =>
          format(subDays(today, i), "yyyy-MM-dd")
        ).reverse();
  ...
  const groupByDate = (category) => {
    ...
    recordings.forEach((recording) => {
      ...
      if (
        recording.analysisResults?.resultDetails === category &&
        occurrencesByDate.hasOwnProperty(date)
      ) {
  ...
  const mainChartOptions = {
    ...
    scales: {
      x: {
        title: { display: true, text: "Categories", color: "#333" },
        grid: { display: false },
      },
      y: {
    ...
    plugins: {
      title: {
        ...
        text: selectedCategory
          ? `Occurrences of ${selectedCategory} Over Time`
  ...
  return (
    <div className="chart-container">
      ...
      {selectedCategory && (
        <div className="flex">
          <button
            ...
            className="back-btn between-charts"
          >
      ...
      <div className="chart-wrapper">
        {selectedCategory ? (
          <Bar
            data={getTimeBasedChartData(selectedCategory)}
            options={getTimeBasedChartOptions}
            id="timeChartId"
          />
        ) : (
